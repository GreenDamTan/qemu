...
static int hax_vcpu_interrupt(CPUArchState *env)
{
    ...
    if (ht->ready_for_interrupt_injection &&
        ...
        if (irq >= 0) {
            ...
            inject_ret = hax_inject_interrupt(env, irq);
            if (!inject_ret) {
                cpu_reset_interrupt(cpu, CPU_INTERRUPT_HARD);
            } else {
                fprintf(stderr, "HAX: failed to inject irq %d: %d\n",
                        irq, inject_ret);
            }
        }
    }
...
static int hax_vcpu_hax_exec(CPUArchState *env)
{
    ...
    if (cpu->halted) {
         ...
         * That way, this vcpu thread will return to the accelerator loop
    ...
    do {
        ...
        case HAX_EXIT_FAST_MMIO:
            ...
            break;
        /* Guest state changed; old HAX reports this as a shutdown request. */
...
int hax_sync_vcpus(void)
{
    if (hax_enabled()) {
        ...
        for (; cpu != NULL; cpu = CPU_NEXT(cpu)) {
            ...
            ret = hax_arch_set_registers(cpu_env(cpu));
            if (ret < 0) {
                return ret;
            }
        }
...
static void hax_accel_class_init(ObjectClass *oc, const void *data)
{
